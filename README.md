...
UI だけ確認したいときは `.env` で `DISCORD_BOT_DISABLED=true` にすると、Bot 接続なしで Web 画面だけ起動できます。

## 必須環境変数

- `DISCORD_TOKEN`: Bot トークン
- `DISCORD_CLIENT_ID`: Discord Application の Client ID
- `DISCORD_GUILD_ID`: 開発用サーバー ID
...
- Bot が起動してもコマンドが見えない場合は `DISCORD_GUILD_ID` を確認してください
